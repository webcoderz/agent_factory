...
def main() -> None:
    ...
    ap.add_argument(
        "--max-parallel-subagents", type=int, default=None, help="Max concurrent subagents (default from env or 4)"
    )
    ap.add_argument(
        "--max-parallel-model-calls", type=int, default=None, help="Max concurrent model calls (default from env or 2)"
    )
    ...
    use_model = args.use_openai_chat_model or bool(
        os.getenv("USE_OPENAI_CHAT_MODEL", "").strip().lower() in ("1", "true", "yes")
    )
    model = None
    if use_model:
        cfg = model_from_env()
    ...
    max_sub = (
        ...
        else int(os.getenv("MAX_PARALLEL_SUBAGENTS", "4"))
    )
    max_llm = (
        args.max_parallel_model_calls
        if args.max_parallel_model_calls is not None
        else int(os.getenv("MAX_PARALLEL_MODEL_CALLS", "2"))
    )
    ctx = build_ctx(
